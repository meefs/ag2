...
!!! tip
    `AssemblerMiddleware` and `AssemblyPolicy` are also available as public symbols from `#!python autogen.beta` if you need to wire the middleware manually (for example, inside a custom harness that isn't built on `Agent`).
