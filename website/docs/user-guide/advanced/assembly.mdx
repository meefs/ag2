...
A reduction cuts history at an index and sends everything after it, and a provider only accepts that tail if it can stand on its own. Three retained events cannot:
